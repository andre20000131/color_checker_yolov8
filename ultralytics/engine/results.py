...
class Results(SimpleClass):
    ...
    def plot(
        ...
        if pred_masks and show_masks:
            ...
            masked_coords = pred_masks.xy
            #print("1111{}".format(masked_coords))
            #min_x = torch.min(masked_coords[0,:,:])
            #print(min_x)
            #max_x = torch.max(masked_coords[0])
            #min_y = torch.min(masked_coords[1])
            #max_y = torch.max(masked_coords[1])


            #pt1 = (min_x, min_y)  # 左上角的点
            #pt2 = (max_x, min_y)  # 右上角的点
            #pt4 = (max_x, max_y)  # 右下角的点
            #pt3 = (min_x, max_y)


            #seg_tensor_list.append(pt1,pt2,pt3,pt4)
        ...
        if pred_boxes is not None and show_boxes:
            for d in reversed(pred_boxes):
                ...
                tensor_list.append(box)
                print("2222{}".format(box))


                #box2 = d.xyxyxyxy.reshape(-1, 4, 2).squeeze()
        ...
        return annotator.result(),tensor_list,masked_coords
